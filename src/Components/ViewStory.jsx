...
export default function ViewStory() {
    ...
    async function fetchStory() {
      try {
        ...
        const response = await axios.get(
          `https://blog-backend-lfz3.onrender.com/api/story/${storyId}`,
          {
            headers: {
  ...
  return (
    <Container fluid className="full-screen-container p-5 ">
      {loading ? (
        ...
        <Card className="view-story-card p-3">
          {story.image && (
          ...
          <Card.Body>
            ...
            <h3 className=" colortitle fs-1 fw-bold pb-1 ">{story.title}</h3>
            <h6 className="card-subtitle mb-2 text-muted fs-3 pt-2 bg-body-secondary">{story.summary}</h6>
            <div>
              {story.content.split("\n\n").map((paragraph, index) => (
                <p key={index} style={{ textAlign: "justify" }} className="fs-5 pt-1">
                  {paragraph}
                </p>
              ))}
            </div>
